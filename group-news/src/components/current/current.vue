    ...
    <scroller lock-x height="-90px" @on-scroll-bottom="onScrollBottom" ref="scrollerBottom"
      ...
      <div>
        <swiper auto loop dots-class="custom-bottom" :aspect-ratio="0.5" dots-position="center">
          <swiper-item class="swiper-demo-img" v-for="(item, index) in imgList" :key="index"><img class="img-responsive"
                                                                                                  :src="item.imgUrl">
          </swiper-item>
          <!--<swiper-item class="swiper-demo-img"><img src="http://placehold.it/450x300/fc0/ccc.png"></swiper-item>-->
          <!--<swiper-item class="swiper-demo-img"><img src="http://placehold.it/450x300/c0f/ccc.png"></swiper-item>-->
        </swiper>
        <div>
          <div v-for="(item,index) in list" class="item clearfix" @click="read(item.link_url,item.id)">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      getMessage () {
        ApiService.get('/api/v2/groupNews/newsList?categoryid=307&page=' + this.page).then(res => {
          this.show = false
      ...
      getImg () {
        ApiService.get('/api/v2/groupNews/imgList?categoryid=307').then(res => {
